...
def calculate_cross_gradient_vectorized(ms, mg, dx, dy, dz):
    """
    使用向量化计算交叉梯度，速度极快，无需多进程。
    输入: 3D 数组 ms (速度), mg (密度)
    输出: 3D 数组 tx, ty, tz (交叉梯度分量)
    注意：输出的尺寸会比输入在每个维度少 2 (因为中心差分处理不了边界)
    """
    
    # 1. 计算 ms 的梯度 (Gradient of S-wave velocity)
    # 沿 Y 轴 (dim 0), X 轴 (dim 1), Z 轴 (dim 2) 的导数
    # 假设输入数据排列是 [ny, nx, nz] (根据您之前的脚本逻辑)
    
    # 注意：这里使用切片 m[2:] - m[:-2] 实现了 m(i+1) - m(i-1)
    
    # dms_dy (对应索引 0)
    dms_dy = (ms[2:, 1:-1, 1:-1] - ms[:-2, 1:-1, 1:-1]) / (2 * dy)
    # dms_dx (对应索引 1)
    dms_dx = (ms[1:-1, 2:, 1:-1] - ms[1:-1, :-2, 1:-1]) / (2 * dx)
    # dms_dz (对应索引 2)
    dms_dz = (ms[1:-1, 1:-1, 2:] - ms[1:-1, 1:-1, :-2]) / (2 * dz)

    # 2. 计算 mg 的梯度 (Gradient of Gravity density)
    dmg_dy = (mg[2:, 1:-1, 1:-1] - mg[:-2, 1:-1, 1:-1]) / (2 * dy)
    dmg_dx = (mg[1:-1, 2:, 1:-1] - mg[1:-1, :-2, 1:-1]) / (2 * dx)
    dmg_dz = (mg[1:-1, 1:-1, 2:] - mg[1:-1, 1:-1, :-2]) / (2 * dz)

    # 3. 计算叉积 (Cross Product): t = grad(ms) x grad(mg)
    # tx = dy * dz - dz * dy
    tx = dms_dy * dmg_dz - dms_dz * dmg_dy
    # ty = dz * dx - dx * dz
    ty = dms_dz * dmg_dx - dms_dx * dmg_dz
    # tz = dx * dy - dy * dx
    tz = dms_dx * dmg_dy - dms_dy * dmg_dx

    # 4. 填充回原始尺寸 (边界补0)
    # 这样保证和原始模型 shape 一致，方便 reshape
    full_tx = np.zeros_like(ms)
    full_ty = np.zeros_like(ms)
    full_tz = np.zeros_like(ms)
    
    full_tx[1:-1, 1:-1, 1:-1] = tx
    full_ty[1:-1, 1:-1, 1:-1] = ty
    full_tz[1:-1, 1:-1, 1:-1] = tz
    
    return full_tx, full_ty, full_tz
...
def main():
    ...
    tx0, ty0, tz0 = calculate_cross_gradient_vectorized(ms_0, mg_0, 1, 1, 1)
