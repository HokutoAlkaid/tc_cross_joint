...
#psxy $path/psclip.txt   -J${PROJ} -R${LATLON} -O -K -L -W1p,blue  >>${FNAME}
...
#psxy CN-faults.dat -W1.0p,white,- -m -R -J -O -K >> ${FNAME}

#psxy gdborder.dat -W1.0p,black -R -m -J -O -K >> ${FNAME}

#pstext faultnm.txt -J${PROJ} -R${LATLON}  -G0/0/0 -O -P -V -K >> ${FNAME}
...
psscale  -B20f20/:mgal:  -D5.6i/2.2i/4.5i/0.2i -C${cptfile}  -O  -K  -S >> ${FNAME}


#echo 99.0 22.5 18 0 0 CM $label| pstext -J${PROJ} -R${LATLON}  -O -P -V -K >> ${FNAME}
echo 115.0 21.5 18 0 0 CM $depth| pstext -J${PROJ} -R${LATLON}  -O -P -V >> ${FNAME}
