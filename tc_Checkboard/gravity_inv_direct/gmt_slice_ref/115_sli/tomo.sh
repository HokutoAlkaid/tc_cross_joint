...
surface ${velfile} -G${grdfile}  -I0.1/0.5 -R${LATLON} -V 
...
grdimage ${grdfile} -J${PROJ} -R${LATLON} -B${TICS} -C${cptfile} -E300 -G -P -K  > ${FNAME}

#echo  22.2 100 16 0 0 LM "(d)" | pstext -J${PROJ} -R${LATLON} -Gblack -O -P -V -K >> ${FNAME}
#echo  27.2 100 16 0 0 LM P2 | pstext -J${PROJ} -R${LATLON} -Gblack -O -P -V  >> ${FNAME}

grdcontour ${grdfile} -J${PROJ} -R${LATLON} -B${TICS} -K -P -O -C0.2 -A0.2f8 \
           -Gd8c  -Wathinner,blue,a -Wcthinner,blue,a -V >> ${FNAME}

echo 23.2 3 12 0 0 LM "LVZ"| pstext -J${PROJ} -R${LATLON}  -O -P -K -V  >> ${FNAME}  
echo 23.5 18 12 0 0 LM "LVZ"| pstext -J${PROJ} -R${LATLON}  -O -P -K -V  >> ${FNAME} 
#echo 23.2 38 12 0 0 LM "HVZ"| pstext -J${PROJ} -R${LATLON}  -O -P -K -V  >> ${FNAME} 

psscale  -B0.2/:km/s:   -D1.8i/-0.35i/3.0i/0.15ih -C${cptfile}  -O  >> ${FNAME}
