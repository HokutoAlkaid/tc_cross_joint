...
grdimage ${grdfile} -J${PROJ} -R${LATLON} -B${TICS} -C${cptfile} -E300 -G -K -P  > ${FNAME}
...
echo 113 3.4 12 0 0 LM "LVZ"| pstext -J${PROJ} -R${LATLON}  -O -P -K -V  >> ${FNAME}
#echo 114 38 12 0 0 LM "HVZ"| pstext -J${PROJ} -R${LATLON}  -O -P -K -V  >> ${FNAME}
           
#gawk '{print $1,$3}' 22.5.txt | psxy  -J${PROJ} -R${LATLON} -O -K -W0.5p,black  >>${FNAME}
