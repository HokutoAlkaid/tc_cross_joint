...
grdfile_ori="yn_bouguer_2m.grd"
grdfile="yn_bouguer_s50_3m.grd"
rm $grdfile
grdfilter $grdfile_ori -D1 -Fg50 -I3m -G$grdfile

grd2xyz $grdfile > 1.txt
cat 1.txt | sort -k1n -k2nr > 2.txt
cat 2.txt | gawk '{printf "%10.3f %10.3f %12.6f \n", $1,$2,$3}' > yn_bouguer_s50_3m.txt
rm 1.txt 2.txt
...
psscale  -B100f50/:mgal:  -D5.6i/2.4i/4.8i/0.2i -C${cptfile}  -O  -K  -S >> ${FNAME}
