#!/bin/sh
#####
#    The purpose of the Script is to plot the tomography result of Yunnan Region
#####
...
#####
FNAME="indochina_bouguer_s15_2m.ps"
#####
...
#cptfile="PALET_WGM_Bouguer_Global.cpt"
grdfile_txt="indochina_bouguer_ori.txt"
grdfile_ori="indochina_bouguer_ori.grd"
grdfile="indochina_bouguer_s15_2m.grd"

xyz2grd $grdfile_txt -R${LATLON} -I1m -G$grdfile_ori

rm $grdfile
grdfilter $grdfile_ori -D1 -Fg15 -I1m -G$grdfile

#---
#   remove the mean value of gravity data
#---
grd2xyz $grdfile > 1.txt

mgra=`cat 1.txt | gawk '{sum+=$3} END {print sum/NR}'`

echo $mgra > mean_grav.txt

awk -v mean=$mgra '{print $1,$2,$3-mean}' 1.txt > 1n.txt

xyz2grd 1n.txt -R${LATLON} -I1m  -G$grdfile

#
#
#

cat 1n.txt | sort -k1n -k2nr > 2.txt
cat 2.txt | gawk '{printf "%10.3f %10.3f %12.6f \n", $1,$2,$3}' > indochina_bouguer_s15_1m.txt
rm 1.txt 1n.txt 2.txt

velscale="-170/150/10"
#cptfile="PALET_WGM_Bouguer_Global.cpt"
cptfile="dw.cpt"
makecpt -Crainbow -T${velscale} -I -Z > ${cptfile}
...
grdimage ${grdfile} -J${PROJ} -R${LATLON} -B${TICS} -C${cptfile} -G -K -P   > ${FNAME}

#pscoast -J${PROJ} -R${LATLON} -B${TICS} -N${BDRYS}  -Dh -G200 -W0.25p -A100 -P -K -O  >> ${FNAME}
pscoast -J${PROJ} -R${LATLON} -B${TICS} -N${BDRYS}  -Dh  -W0.25p -A1000 -P -O -K   >> ${FNAME}
#psclip $path/psclip.txt -J${PROJ} -R${LATLON} -K -P -O  >>${FNAME}

psxy chinaborder/CN-faults.dat -W1.0p,black -m  -R -J -O -K >> ${FNAME}
...
#psxy grdcontour_neg3000.txt -W1.0p,red  -R -J -O -K >> ${FNAME}

#psxy Khorat_Plateau.txt -W1.5p,white -L  -R -J -O -K >> ${FNAME}
...
psscale  -B50f50/:mgal:  -D5.6i/2.4i/4.8i/0.2i -C${cptfile}  -O   -S >> ${FNAME}


#echo 98.5 22.5 24 0 0 CM $label| pstext -J${PROJ} -R${LATLON}  -O -P -V >> ${FNAME}
...
#cat $path/tectonic.dat | gawk '{print $1,$2}' | psxy -J${PROJ} -R${LATLON} -Sc0.15i  -G255/0/0 -O  -V >>${FNAME}

#psxy $path/citylalon.txt -J${PROJ} -R${LATLON} -Skstar/0.12i  -G255/0/0 -O  -K -V >>${FNAME}
#pstext $path/cityname.txt -J${PROJ} -R${LATLON}  -G255/0/0 -Y0.15 -O -P -K -V  >> ${FNAME}
#pstext $path/tectonic.dat -J${PROJ} -R${LATLON}  -G255/0/0  -O -Y0.15 -P -V >> ${FNAME}
